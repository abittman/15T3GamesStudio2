...
public class BirdController : MonoBehaviour 
{
    ...
    private BirdController[] otherFlocks;       //Unity Flocks in the group
    ...
    void Start ()
    {
        ...
        if (transform.parent)
        {
            tempFlocks = transform.parent.GetComponentsInChildren<BirdController>();
        }

        //Assign and store all the flock objects in this group
	    objects = new Transform[tempFlocks.Length];
        otherFlocks = new BirdController[tempFlocks.Length];

	    for(int i = 0;i<tempFlocks.Length;i++)
	    {
		    objects[i] = tempFlocks[i].transform;
		    otherFlocks[i] = (BirdController)tempFlocks[i];
	    }

        //Null Parent as the flock leader will be BirdControllerController object
	    ...
	    for(int i = 0;i<objects.Length;i++)
	    {
            ...
            if (transform != transformComponent)
		    {
			    ...
			    if (d < followRadius)
			    {
				    ...
				    f = d / followRadius;
				    BirdController tempOtherFlock = otherFlocks[i];
