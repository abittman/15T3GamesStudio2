...
// 初期化
	...
	void FixedUpdate ()
	{
		...
		if (IsPlayerControlled && Input.GetButtonDown("Jump") || IsJump) {	// スペースキーを入力したら
		...
		{
			if (IsPlayerControlled) cameraObject.SendMessage("setCameraPositionJumpView");	// ジャンプ中のカメラに変更
			// ステートがトランジション中でない場合
			if(!anim.IsInTransition(0))
			{
